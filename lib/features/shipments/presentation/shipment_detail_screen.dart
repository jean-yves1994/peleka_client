...
class ShipmentDetailScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading: IconButton(
          ...
          onPressed: () =>
              context.canPop() ? context.pop() : context.go('/shipments'),
        ),
        title: const Text('Shipment'),
        actions: [
          IconButton(
              icon: const Icon(Icons.refresh),
              onPressed: () => ref.invalidate(shipmentDetailProvider(id))),
        ],
      ),
      body: a.when(
        ...
        error: (e, _) => Center(
          child: Padding(
              ...
              child: Text('Could not load shipment.\n\n$e',
                  textAlign: TextAlign.center)),
        ),
        data: (d) {
          final s = d.shipment;
          final active = _active.contains(s.status);
          final standardUnpaid = s.customerType != 'premier' &&
              s.paymentStatus != 'paid' &&
              s.status == 'pending_payment';
          final premierOutstanding =
              s.customerType == 'premier' && s.paymentStatus != 'paid';
          ...
          return RefreshIndicator(
            ...
            child: ListView(padding: const EdgeInsets.all(20), children: [
              ...
              if (standardUnpaid || premierOutstanding) ...[
                PelekaCard(
                  ...
                  child: Column(
                      ...
                      children: [
                        Row(children: [
                          ...
                          Expanded(
                            child: Text(
                                s.customerType == 'premier'
                                    ? 'Premier balance outstanding'
                                    : 'Payment required',
                                style: const TextStyle(
                                    fontSize: 16,
                        ...
                        Text(
                          s.customerType == 'premier'
                              ? (s.status == 'delivered'
                                  ? 'This Premier shipment can be settled now. Your outstanding balance will be reduced after payment.'
                                  : 'This shipment is on your Premier invoice. You can settle it after delivery.')
                        ...
                        if (s.customerType != 'premier' ||
                            s.status == 'delivered') ...[
                          const SizedBox(height: 14),
                          PelekaButton(
                            label:
                                'Pay ${money(s.totalPrice, currency: s.currency)}',
                            icon: Icons.lock,
                            onPressed: () async {
                              await context.push(
              ...
              PelekaCard(
                ...
                _line(Icons.trip_origin, s.pickupAddress,
                    'Pickup · ${s.pickupCity ?? "—"}', AppColors.blue),
                Container(
                    margin: const EdgeInsets.only(left: 20),
                    height: 20,
                    width: 1,
                    color: AppColors.ink200),
                _line(Icons.location_on, s.deliveryAddress,
                    'Delivery · ${s.deliveryCity ?? "—"}', AppColors.orange),
              ])),
              const SizedBox(height: 12),
                      ...
                      return IntrinsicHeight(
                          child: Row(
                            ...
                            Column(children: [
                              ...
                              if (i < d.statusHistory.length - 1)
                                Expanded(
                                    child: Container(
                                        width: 1, color: AppColors.ink200)),
                            ]),
                            const SizedBox(width: 12),
                            Expanded(
                                child: Padding(
              ...
              PelekaCard(
                  child: Column(
                    ...
                    Wrap(spacing: 8, runSpacing: 6, children: [
                      ...
                      if (s.parcelCategory != null)
                        _chip(titleCase(s.parcelCategory!)),
                      if (s.isFragile)
                        _chip('Fragile', color: AppColors.warning),
                    ]),
                  ])),
              const SizedBox(height: 12),
              ...
              PelekaCard(
                  ...
                  child: Column(children: [
                    if (s.discountAmount != null && s.discountAmount! > 0)
                      _pl(
                          'Discount',
                          '−${money(s.discountAmount, currency: s.currency)}',
                          false,
                          color: AppColors.success),
                    _pl('VAT (18%)', money(s.taxAmount, currency: s.currency),
                        false),
                    const Divider(height: 16),
                    _pl(
                      (standardUnpaid || premierOutstanding)
                          ? 'Amount due'
                          : 'Total paid',
                      money(s.totalPrice, currency: s.currency),
                      true,
                    ),
                  ])),
              const SizedBox(height: 20),
                ...
                OutlinedButton.icon(
                  ...
                  onPressed: () async {
                    ...
                    try {
                      ...
                      ref.invalidate(myShipmentsProvider);
                      if (context.mounted)
                        _sn(context, 'Shipment cancelled', true);
                    } catch (e) {
                      if (context.mounted)
                        _sn(
                            context,
                    ...
                    child: Column(
                      ...
                      Row(
                          children: List.generate(
                              5,
                              (i) => Icon(
                                  i < (d.rating!['score'] as int)
                                      ? Icons.star_rounded
                                      : Icons.star_outline_rounded,
                                  color: AppColors.warning,
                                  size: 24))),
                      if (d.rating!['comment'] != null &&
                          (d.rating!['comment'] as String).isNotEmpty) ...[
                        const SizedBox(height: 8),
  ...
  Widget _line(IconData i, String a, String l, Color c) =>
  ...
  Widget _chip(String l, {Color? color}) => Container(
        ...
        decoration: BoxDecoration(
            color: (color ?? AppColors.navy).withOpacity(0.1),
        ...
        child:
          ...
          Text(l,
              style: TextStyle(
                  ...
                  color: color ?? (bold ? AppColors.navy : AppColors.ink700))),
          Text(v,
              style: TextStyle(
                  fontSize: bold ? 17 : 13,
                  fontWeight: bold ? FontWeight.w800 : FontWeight.w600,
                  color: color ?? (bold ? AppColors.orange : AppColors.navy))),
        ]),
      );
    ...
    return showDialog<String>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text('This cannot be undone. Please tell us why:',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () => Navigator.pop(
                c, t.text.trim().isEmpty ? 'Changed my mind' : t.text.trim()),
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      PelekaButton(
                          ...
                          onPressed: () async {
                            try {
                              await ref.read(shipmentRepositoryProvider).rate(
                                  sid,
                                  score: score,
                                  comment: cm.text.trim());
                              ref.invalidate(shipmentDetailProvider(sid));
                              if (ctx.mounted) Navigator.pop(ctx);
                              if (ctx.mounted)
                                _sn(ctx, 'Thanks for the rating!', true);
                            } catch (e) {
                              if (ctx.mounted)
                                _sn(
                                    ctx,
  ...
  void _sn(BuildContext c, String m, bool ok) =>
