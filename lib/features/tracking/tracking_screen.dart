...
class TrackingScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TrackingScreen> createState() => _S();
}
...
class _S extends ConsumerState<TrackingScreen> {
  ...
  void initState() {
    super.initState();
    _load();
    _t = Timer.periodic(const Duration(seconds: 8), (_) => _load());
  }
  ...
  Future<void> _load() async {
    try {
      final d = await ref.read(shipmentRepositoryProvider).track(widget.id);
      if (mounted)
        setState(() {
          _track = d;
          _loading = false;
    ...
    return Scaffold(
        ...
        body: a.when(
            ...
            data: (d) {
              ...
              final delivery = LatLng(s.deliveryLat, s.deliveryLng);
              LatLng? rider;
              final r = _track?['rider_last_location'];
                  ...
                  try {
                    _map.fitCamera(CameraFit.bounds(
                        bounds: LatLngBounds.fromPoints(
                            [pickup, delivery, if (rider != null) rider]),
                ...
                Expanded(
                    child: FlutterMap(
                      ...
                      TileLayer(
                          ...
                          userAgentPackageName: 'com.peleka.client'),
                      PolylineLayer(polylines: [
                        Polyline(
                            points: rider == null
                                ? [pickup, delivery]
                                : [pickup, rider, delivery],
                            strokeWidth: 3,
                      ...
                      MarkerLayer(markers: [
                        Marker(
                            ...
                            width: 40,
                            height: 40,
                            child: _pin(AppColors.blue, Icons.trip_origin)),
                        Marker(
                            point: delivery,
                            width: 40,
                            height: 40,
                            child: _pin(AppColors.orange, Icons.location_on)),
                        if (rider != null)
                          Marker(
                              point: rider,
                              width: 48,
                              height: 48,
                              child: _pin(AppColors.navy, Icons.two_wheeler))
                      ])
                    ])),
                Container(
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          Row(children: [
                            ...
                            const SizedBox(width: 6),
                            Text(
                                'Auto-refreshes every 8s · updated ${relative(DateTime.now())}',
                                style: const TextStyle(
  ...
  Widget _pin(Color c, IconData i) => Container(
